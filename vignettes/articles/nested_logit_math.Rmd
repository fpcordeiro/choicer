...
- $\lambda_k > 1$ is mathematically evaluable in the implemented likelihood but lies outside the usual positive-correlation nested-logit interpretation; it implies negative within-nest correlation and raises the standard random-utility regularity concerns that motivate the common $0 < \lambda_k \leq 1$ restriction.
...
The probabilities in §§2.3–2.4 are the utility-maximization-consistent GEV form (McFadden 1978; Train 2009, Ch. 4): utility is scaled by $1/\lambda_k$ inside the conditional probability, and the inclusive value is raised by $\lambda_k$ in the nest probability. Some software instead estimates the *non-normalized* nested logit, in which $V_{ij}$ enters the within-nest logit unscaled. The two forms imply the same choice probabilities only in special cases (for instance, utilities consisting solely of alternative-specific constants); with generic coefficients the non-normalized form is not consistent with random-utility maximization, and its $\beta$ and $\lambda$ estimates are not comparable to the GEV form's without rescaling (Koppelman & Wen 1998; Hensher & Greene 2002; Heiss 2002). When benchmarking choicer against another package, verify which parameterization the other package implements before comparing coefficients or dissimilarity parameters.
...
*Code reference: `nl_bhhh_parallel()` in [src/nestlogit.cpp](https://github.com/fpcordeiro/choicer/blob/main/src/nestlogit.cpp); `compute_sandwich_vcov()` and `.sandwich_combine()` in [R/classes.R](https://github.com/fpcordeiro/choicer/blob/main/R/classes.R); `wesml_weights()` / `sample_by_choice()` in [R/sampling.R](https://github.com/fpcordeiro/choicer/blob/main/R/sampling.R).*
