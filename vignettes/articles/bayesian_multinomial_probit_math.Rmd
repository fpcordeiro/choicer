---
title: "The math behind choicer: Bayesian multinomial probit"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{The math behind choicer: Bayesian multinomial probit}
...
*Code reference: [src/mnprobit.cpp](https://github.com/fpcordeiro/choicer/blob/main/src/mnprobit.cpp)*
...
*Code reference: [mnprobit_utils.R (prepare_mnp_data)](https://github.com/fpcordeiro/choicer/blob/main/R/mnprobit_utils.R)*
...
$$
E[\Sigma] = \frac{p+3}{2}I_p,
$$

not $I_p$. This is not a contradiction because the raw scale is unidentified and every reported draw is subsequently divided by its own $\sigma_{11}$. More importantly, it is not a reason to call the induced prior on $(\tilde\beta,\tilde\Sigma)$ weak: normalization changes dependence and tail behavior. These defaults should be interrogated through prior-predictive and sensitivity analysis, especially for covariance, correlation, and substitution conclusions when alternative-specific identifying variation is limited. The engine accepts $\nu\ge p$, which is sufficient for a proper inverse-Wishart under this convention; the displayed prior-mean formula exists only for $\nu>p+1$.
...
*Code reference: [src/mnprobit.cpp](https://github.com/fpcordeiro/choicer/blob/main/src/mnprobit.cpp) (Gram blocks), [src/mnprobit.cpp](https://github.com/fpcordeiro/choicer/blob/main/src/mnprobit.cpp) (draw)*

### 4.3 Covariance: $\Sigma \mid w, \beta$

With $\varepsilon_i = w_i - X_i\beta$ and $S = \sum_i \varepsilon_i \varepsilon_i'$, conjugacy of the inverse-Wishart gives

$$
\Sigma \mid w, \beta \;\sim\; \mathrm{IW}\!\left(\nu + N,\; V + S\right).
$$

The implementation accumulates the lower triangle of $S$ in fixed $(i,j,k)$ order, copies it to the upper triangle, and symmetrizes $V+S$ before factorization. The fixed-order accumulation is deliberate: it avoids a threaded BLAS reduction whose rounding order could vary with the OpenMP configuration.
...
To draw from $\mathrm{TN}(\mu, \sigma^2; a, b)$, standardize to bounds $(\alpha, \beta_u) = ((a-\mu)/\sigma, (b-\mu)/\sigma)$ and, when $\beta_u<0$, use normal symmetry to mirror the interval into the upper half-line. The current implementation selects among three exact algorithms. Write $h=\beta_u-\alpha$.

- **Wide central interval** ($h>0.5$ and $\alpha<0.45$): draw $z\sim N(0,1)$ until $\alpha<z<\beta_u$. In this region ordinary rejection avoids the relatively expensive `pnorm`/`qnorm` round trip while retaining adequate acceptance probability. This is the common MNP augmentation case.

- **Wide or ultra-deep-tail interval** ($h>0.5$ or $\alpha>30$, outside the preceding case): use Robert's (1995) exponential rejection. With rate

$$
\lambda = \frac{\alpha + \sqrt{\alpha^2 + 4}}{2},
$$

propose $z = \alpha + E/\lambda$ with $E \sim \mathrm{Exp}(1)$, reject $z > \beta_u$, and accept with probability $\exp\!\left(-(z - \lambda)^2 / 2\right)$. This remains stable arbitrarily far into the tail.

- **Narrow interval** ($h\le0.5$ and $\alpha\le30$): inverse CDF on the upper-tail scale,

$$
z = \bar\Phi^{-1}(u),\qquad
u\sim\mathrm{Uniform}\!\left(\bar\Phi(\beta_u),\bar\Phi(\alpha)\right).
$$

Narrow intervals can make rejection stall. Upper-tail probabilities avoid the loss of precision that occurs when $\Phi(x)$ rounds to one in the positive tail.
...
*Code reference: [src/bayes_samplers.h](https://github.com/fpcordeiro/choicer/blob/main/src/bayes_samplers.h)*

### 5.2 Standard Normal, Gamma, Chi-Squared

- **Normal**: Marsaglia polar method — exact, no tables; generates pairs and caches the spare. *Code reference: [src/rng.h](https://github.com/fpcordeiro/choicer/blob/main/src/rng.h)*
- **Gamma**$(a, 1)$: Marsaglia & Tsang (2000) squeeze method — exact for any $a \geq 1$; for $a < 1$ the boosting identity $X = \mathrm{Gamma}(a+1)\, U^{1/a}$ is used. *Code reference: [src/rng.h](https://github.com/fpcordeiro/choicer/blob/main/src/rng.h)*
- **Chi-squared**$(k)$ $= 2\,\mathrm{Gamma}(k/2, 1)$, valid for non-integer $k$ (needed by Bartlett). *Code reference: [src/rng.h](https://github.com/fpcordeiro/choicer/blob/main/src/rng.h)*
