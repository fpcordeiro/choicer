...
### 2.5 Statistical Properties of the MSL Estimator

The simulated probability $\bar{P}_{ij_i}$ is an unbiased simulator of the true mixed-logit probability, but the log-likelihood uses its *logarithm*, and by Jensen's inequality $\mathbb{E}[\log \bar{P}] < \log \mathbb{E}[\bar{P}]$. The simulated log-likelihood is therefore biased for any finite number of draws, and the MSL estimator inherits a bias of order $O(1/S)$ — the second-order term of the expansion of $\log \bar{P}$ around $P$, proportional to the simulation variance of $\bar{P}$. The classical results (Hajivassiliou & Ruud 1994; Lee 1995; Gouriéroux & Monfort 1996; Train 2009, Ch. 10) are:
...
- **$S$ rising faster than $\sqrt{N}$**: MSL is asymptotically equivalent to exact maximum likelihood — the simulation noise is $o_p(N^{-1/2})$, the $\sqrt{N}$-normal limit distribution is that of the exact MLE, and the usual Hessian/BHHH variance estimators apply.

Quasi-Monte Carlo draws improve the constants, not these conditions: Halton sequences and their Owen-scrambled randomization (§5.4) reduce the simulation variance of $\bar{P}$ at a given $S$ by an order of magnitude or more relative to pseudo-random draws (Bhat 2003), which is why a few hundred Halton draws often match thousands of pseudo-random ones. The practical discipline follows directly — treat $S$ as part of the specification: increase it until the estimates *and* their standard errors stabilize, and if the parameters of interest (typically the Cholesky block) still move with $S$, report that sensitivity as part of the result. `mc_asymptotics()` operationalizes the consistency, asymptotic-normality, coverage, and information-matrix-equality claims as testable Monte Carlo diagnostics.
...
*Code reference: `compute_sandwich_vcov()` and `.sandwich_combine()` in [R/classes.R](https://github.com/fpcordeiro/choicer/blob/main/R/classes.R); `wesml_weights()` / `sample_by_choice()` in [R/sampling.R](https://github.com/fpcordeiro/choicer/blob/main/R/sampling.R).*
