...
(`run_nestlogit()`) fits. choicer also offers Bayesian samplers: a
...
| **Multinomial probit** | A covariance matrix for utility-difference shocks. | Scale normalization, a rapidly growing covariance parameterization, and MCMC diagnostics. Alternative-specific regressors are the exclusion restrictions that identify the covariance (Keane 1992). choicer does not yet provide MNP prediction or a rule for transporting covariance to counterfactual choice sets. |
| **Hierarchical MNL** | Panel-identified taste heterogeneity $\beta_i \sim N(b, W)$ plus partially-pooled alternative effects $\delta_j = z_j'\theta + \xi_j$, with iid EV1 choice shocks. | The priors, MCMC convergence, and—for entry counterfactuals—the exchangeability of entrant residual quality with incumbents. Repeated choices per person are the variation that earns the taste distribution. |
...
- **Start from the estimand.** Is the target a slope, WTP ratio, substitution pattern,
   population share, welfare contrast, or entry effect? If the object is an own-price elasticity on the
...
- **Understand the restrictions.** What is normalized, excluded, pooled, or supplied
   by the error distribution, mixing distribution, or nesting tree?

- **Identifying variation:** Which within-choice-set, across-market, panel, or
   sampling-design variation identifies the extra flexibility over the baseline?
...
- **Inference:** Are observations independent, clustered, weighted by a
   choice-based design, or summarized by posterior uncertainty?
...
- **Estimation stress test:** Are results stable across scaling, starts,
   simulation draws, MCMC seeds/chains, and plausible specifications?

One question is prior to everything in the table: is the price coefficient
itself credibly identified? If prices respond to unobserved demand — the
standard concern whenever unobserved quality feeds both purchases and pricing —
