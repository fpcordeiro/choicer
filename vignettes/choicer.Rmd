...
| **Multinomial probit** | A covariance matrix for utility-difference shocks. | Scale normalization, a rapidly growing covariance parameterization, MCMC diagnostics and a clear rule for extending the covariance structure in counterfactual choice sets. Alternative-specific regressors are the exclusion restrictions that identify the covariance (Keane 1992). |
| **Hierarchical Bayes (HMNL/HMNP)** | Panel-identified taste heterogeneity $\beta_i \sim N(b, W)$ plus partially-pooled alternative effects $\delta_j = z_j'\theta + \xi_j$. | The priors, MCMC convergence, and — for entry counterfactuals — the exchangeability of the entrant's unobserved quality $\xi$ with the incumbents'. Repeated choices per person are the variation that earns the taste distribution. |
