...
- **Respondent level.** Each person carries one taste vector $\beta_i$ across
  ...
  which every choice situation gets an independent taste draw (see the
  ...
  is precisely the variation that identifies the taste distribution $W$, and
...
for a serious empirical run, four or more independently seeded chains are often
a better default. Report the number of people, tasks per person and alternatives;
...
posterior, returning credible intervals (and the outside share) for free:
...
use the same posterior and taste draws, so the contrast is exact:
...
Unlike a delta-method standard error, this interval inherits the full
posterior uncertainty — including the heterogeneity in the price coefficient
that sits in the denominator. A wide CV interval is not a bug; it is the
model telling you how much the welfare number depends on tastes the data pin
down imperfectly. (`logsum()` and `consumer_surplus()` are available for the
hierarchical *logit* only; the probit expected-maximum counterpart is on the
roadmap.)
...
its credible interval is much wider than the incumbents' — properly so. The
...
uncertainty about unobserved quality *is* the dominant uncertainty about an
entrant, and the posterior predictive says so rather than hiding it. The
...
the EV1 shock has standard deviation $\pi/\sqrt{6} \approx 1.28$ against the
probit's 1, so multiplying the probit coefficients by $\pi/\sqrt{6}$ puts the
two models' estimates on comparable footing:
...
The `choicer_hb` post-estimation suite — `predict()` (probabilities by exact
one-dimensional Gauss-Hermite quadrature), `wtp()`, `elasticities()`,
`diversion_ratios()`, `ppc_shares()`, `recovery_table()` — works identically
on the probit fit. The exceptions are `logsum()` and `consumer_surplus()`,
