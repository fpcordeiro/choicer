...
simulated likelihood: each choice situation receives its own taste draw. It is
not a frequentist panel mixed-logit likelihood.
...
> [math companion](mixed_logit_math.html) for the conditions). If the solver
> struggles, pass an explicit `theta_init` (for example the MNL coefficients)
> and bounds on the Cholesky diagonal. See
...
- **$f(\beta)$ is hard to identify and estimate in practice.** A single
  ...
  still maximizes a *cross-sectional* simulated likelihood — each choice
  situation gets an independent taste draw. Cluster-robust standard errors
