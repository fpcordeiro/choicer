...
Computes the BHHH approximation to the observed information matrix for
the Mixed Logit model: \\H\_{BHHH} = \sum_i w_i \cdot s_i s_i^\top\\,
where \\s_i\\ is the per-individual score (gradient of \\\log
\bar{P}\_i\\). This outer product of gradients (OPG) estimator provides
an alternative to the analytical Hessian for standard error computation
that scales to large problems where the analytical Hessian is infeasible
(e.g., many alternatives or simulation draws).
...
as the negated Hessian returned by `mxl_hessian_parallel`, so it can be
inverted directly to obtain vcov).

## Note

The BHHH/OPG estimator is only asymptotically equivalent to the
Hessian-based information matrix at the true MLE. In finite samples it
can underestimate standard errors, particularly when the model is
mis-specified or away from the optimum.
