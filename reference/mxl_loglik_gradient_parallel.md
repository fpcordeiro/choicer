...
Computes the log-likelihood and its gradient for the Mixed Logit model
using OpenMP for parallelization. Allows for inclusion of
alternative-specific constants, outside option, observation weights,
...
- rc_mean:
  ...
  whether to estimate means for random coefficients. If so, mean
  parameters (mu) should be included in theta after beta parameters.

- use_asc:

  whether to use alternative-specific constants. If so, parameters
  should be included in theta after beta and L (and mu, if applicable).
